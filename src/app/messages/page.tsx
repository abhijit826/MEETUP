...
function MessagesContent() {
  ...
  return (
    <main className="min-h-screen flex flex-col items-center justify-center p-0 sm:p-4 bg-slate-100/70 dark:bg-slate-950 text-slate-900 dark:text-slate-100 transition-colors duration-200">
      {/* Toast Alert */}
      {toastMessage && (
        <div className="fixed top-5 z-50 px-5 py-3 rounded-2xl bg-gray-900 dark:bg-slate-800 text-white text-xs font-bold shadow-2xl flex items-center gap-2 animate-fade-in border border-slate-700">
          <CheckCircle2 size={15} className="text-emerald-400 shrink-0" />
          <span>{toastMessage}</span>
        </div>
      )}

      <div className="w-full max-w-[440px] h-screen sm:h-[880px] bg-white dark:bg-slate-900 sm:rounded-3xl sm:border sm:border-gray-200 dark:sm:border-slate-800 sm:shadow-2xl flex flex-col overflow-hidden">
        {/* VIEW 1: CONVERSATION LIST */}
        {!activeConvId ? (
          <div className="flex-1 flex flex-col h-full">
            {/* Header */}
            <header className="p-4 border-b border-gray-100 dark:border-slate-800 flex items-center justify-between">
              <div className="flex items-center gap-3">
                <Link
                  href="/confessions"
                  className="w-9 h-9 rounded-full flex items-center justify-center bg-gray-100 dark:bg-slate-800 hover:bg-gray-200 dark:hover:bg-slate-700 transition-all text-gray-700 dark:text-slate-200"
                >
                  <ArrowLeft size={18} />
                </Link>
                <div>
                  <h1 className="font-black text-xl text-gray-900 dark:text-white tracking-tight flex items-center gap-1.5">
                    Messages
                    <span className="text-xs py-0.5 px-2 rounded-full bg-indigo-100 dark:bg-indigo-950/60 text-[#4F46E5] dark:text-indigo-300 font-bold border border-indigo-200/50 dark:border-indigo-800/40">
                      Private
                    </span>
                  </h1>
                  <p className="text-xs text-gray-500 dark:text-slate-400">Anonymous &amp; Direct Chat</p>
                </div>
              </div>
            </header>
            ...
            <div className="p-3 border-b border-gray-100 dark:border-slate-800">
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full h-10 pl-10 pr-4 rounded-xl border border-gray-200 dark:border-slate-700 text-xs outline-none focus:border-[#4F46E5] bg-gray-50/50 dark:bg-slate-800/70 text-slate-900 dark:text-white"
                />
              </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto divide-y divide-gray-50 dark:divide-slate-800/60">
              {filteredConversations.length === 0 ? (
                <div className="text-center py-16 px-4 flex flex-col items-center gap-3">
                  <MessageCircle size={36} className="text-gray-300 dark:text-slate-700" />
                  <p className="font-bold text-sm text-gray-800 dark:text-slate-200">No private messages yet</p>
                  <p className="text-xs text-gray-400 dark:text-slate-500 max-w-[240px]">
                ...
                filteredConversations.map((conv) => {
                  ...
                  return (
                    <div
                      ...
                      className="p-4 hover:bg-gray-50/80 dark:hover:bg-slate-800/60 transition-all cursor-pointer flex items-center gap-3.5"
                    >
                      {/* Avatar */}
                      <div
                        className={`relative w-12 h-12 rounded-full flex items-center justify-center font-black text-sm shrink-0 ${
                          isAnon
                            ? "bg-indigo-50 dark:bg-indigo-950/60 text-[#4F46E5] dark:text-indigo-300 border border-indigo-100 dark:border-indigo-800/50"
                            : "bg-coral-50 dark:bg-rose-950/60 text-[#EF4444] dark:text-rose-300 border border-coral-100 dark:border-rose-800/50"
                        }`}
                      >
                        {isAnon ? (
                          <ShieldCheck size={22} />
                        ...
                        {conv.unreadCount > 0 && (
                          <span className="absolute top-0 right-0 w-3.5 h-3.5 bg-red-500 border-2 border-white dark:border-slate-900 rounded-full" />
                        )}
                      </div>
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between mb-0.5">
                          <div className="flex items-center gap-1.5">
                            ...
                            <span className="text-[10px] font-bold py-0.5 px-1.5 rounded-md bg-gray-100 dark:bg-slate-800 text-gray-600 dark:text-slate-300 shrink-0">
                              {college}
                            </span>
                          </div>
                        ...
                        {conv.confessionSnippet && (
                          <p className="text-[11px] text-[#4F46E5] dark:text-indigo-300 font-semibold truncate mb-1 bg-indigo-50/60 dark:bg-indigo-950/50 px-2 py-0.5 rounded-md border border-indigo-100/50 dark:border-indigo-800/40">
          ...
          <div className="flex-1 flex flex-col h-full bg-white dark:bg-slate-900 relative">
            {/* Header */}
            {activeConversation && (
              <header className="p-3.5 border-b border-gray-100 dark:border-slate-800 flex items-center justify-between bg-white dark:bg-slate-900 z-10 shrink-0">
                <div className="flex items-center gap-2.5">
                  <button
                    onClick={() => setActiveConvId(null)}
                    className="w-8 h-8 rounded-full flex items-center justify-center bg-gray-100 dark:bg-slate-800 hover:bg-gray-200 dark:hover:bg-slate-700 transition-all text-gray-700 dark:text-slate-200"
                  >
                    <ArrowLeft size={16} />
                  </button>
                  ...
                  {(() => {
                    ...
                    return (
                      <>
                        ...
                        <div>
                          <div className="flex items-center gap-1.5">
                            <h2 className="font-extrabold text-sm text-gray-900 dark:text-white leading-tight">
                              {displayName}
                            </h2>
                            <span className="text-[10px] font-bold py-0.2 px-1.5 rounded bg-gray-100 dark:bg-slate-800 text-gray-600 dark:text-slate-300">
                              {college}
                            </span>
                          </div>
                ...
                <div className="flex items-center gap-1 relative">
                  ...
                  <button
                    ...
                    className="w-8 h-8 rounded-full flex items-center justify-center text-gray-500 dark:text-slate-400 hover:bg-gray-100 dark:hover:bg-slate-800 transition-all"
                  >
                    <MoreVertical size={16} />
                  </button>

                  {showOptionsMenu && (
                    <div className="absolute right-0 top-10 w-44 rounded-2xl bg-white dark:bg-slate-800 border border-gray-100 dark:border-slate-700 shadow-xl p-1 z-30 animate-fade-in text-xs font-semibold text-gray-700 dark:text-slate-200">
                      <button
                        onClick={() => {
                          setShowOptionsMenu(false);
                      ...
                      <button
                        ...
                        className="w-full px-3 py-2 text-left rounded-xl hover:bg-gray-50 dark:hover:bg-slate-700 flex items-center gap-2 text-gray-800 dark:text-slate-200"
                      >
                        <Ban size={14} className="text-gray-500 dark:text-slate-400" />
                        <span>
                      ...
                      <button
                        ...
                        className="w-full px-3 py-2 text-left rounded-xl hover:bg-gray-50 dark:hover:bg-slate-700 flex items-center gap-2 text-red-600 dark:text-rose-400"
                      >
                        <Flag size={14} />
            ...
            {activeConversation?.confessionSnippet && (
              <div className="p-3 bg-indigo-50/70 dark:bg-indigo-950/40 border-b border-indigo-100/60 dark:border-indigo-800/40 text-xs text-indigo-950 dark:text-indigo-200 flex items-start gap-2 shrink-0">
                <Lock size={14} className="text-[#4F46E5] dark:text-indigo-400 shrink-0 mt-0.5" />
                <p className="line-clamp-2">
                  <span className="font-bold text-[#4F46E5] dark:text-indigo-400">Confession Context: </span>
                  &quot;{activeConversation.confessionSnippet}&quot;
                </p>
              </div>
            )}
            ...
            {/* Message Area */}
            <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-3 bg-slate-50/50 dark:bg-slate-950/40">
              {messages.map((msg) => {
                const isMe =
                    ...
                    <div
                      className={`p-3.5 rounded-2xl text-xs font-medium leading-relaxed shadow-sm ${
                        isMe
                          ...
                          : "bg-white dark:bg-slate-800 text-gray-900 dark:text-slate-100 border border-gray-100 dark:border-slate-700 rounded-bl-none"
                      }`}
                    >
                      {msg.content}
                    </div>
            ...
            {activeConversation?.isBlocked ? (
              <div className="p-4 bg-gray-100 dark:bg-slate-800 text-center text-xs text-gray-500 dark:text-slate-400 font-semibold border-t border-gray-200 dark:border-slate-700">
                🚫 You have blocked this user. You cannot send or receive messages.
              </div>
            ) : (
              <form
                onSubmit={handleSendMessage}
                className="p-3 border-t border-gray-100 dark:border-slate-800 bg-white dark:bg-slate-900 flex items-center gap-2 shrink-0"
              >
                <input
                  type="text"
                  value={inputText}
                  ...
                  required
                  className="flex-1 h-11 px-4 rounded-2xl border border-gray-200 dark:border-slate-700 text-xs outline-none focus:border-[#4F46E5] dark:focus:border-indigo-500 transition-all bg-slate-50/50 dark:bg-slate-800/80 text-slate-900 dark:text-white"
                />
                <Button
                  type="submit"
                  variant="primary"
